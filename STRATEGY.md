...
| **US13** ✅ | Le tempo qu'on règle | *Livrée.* La grille existait depuis l'US7 et **rien ne l'exposait** : 96 BPM figés. Le bouton de tempo est une **glissière** — on appuie, on glisse, la valeur suit — plutôt qu'une roue de préréglages, parce que le tempo voyage déjà comme valeur **continue** dans le lien : des paliers rendraient inaffichable un lien reçu à 103 BPM. Deux défauts trouvés en le vérifiant : une source gardait une échéance périmée après un changement de tempo — 0,9 s d'attente pour une période de 0,714 s, donc la promesse « sans rafale » de l'US7 ne tenait qu'après la première émission — et un lien reçu affichait 96 BPM alors que l'état valait 144. Requirements : `docs/plans/us13-le-tempo-quon-regle.md`. |
